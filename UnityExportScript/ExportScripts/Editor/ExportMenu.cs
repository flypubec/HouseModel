		...
		public class JsonMesh{
			...
			*/
			...
			public JsonMesh(Mesh mesh, Exporter exp){
				...
				path = filePath;
                name = Path.GetFileNameWithoutExtension(filePath)+"_"+ mesh.name;
...
//				useSpecular = mat.IsKeywordEnabled("_SPECCGLOSSMAP");
                ...
                if (shader == "Standard (Specular setup)")
                {
                    useMetallic = false;
                    useSpecular = true;
                }
                ...
                color = getColor("_Color", Color.white);
                albedoTex = getTexId("_MainTex");
                if (albedoTex == -1)
                    albedoTex = getTexId("_Albedo");
                if (mainTexture == -1)
                    mainTexture = albedoTex;
                specularTex = getTexId("_SpecGlossMap");
				metallicTex= getTexId("_MetallicGlossMap");
                if (metallicTex == -1)
                    metallicTex = getTexId("_MetallicGloss");
                    if (metallicTex == -1)
                        metallicTex = getTexId("_Metallic");
                if (metallicTex != -1)
                    useMetallic = true;

                normalMapTex = getTexId("_BumpMap");
                if (normalMapTex == -1)
                    normalMapTex = getTexId("_Normal");
                    if (normalMapTex == -1)
                        normalMapTex = getTexId("_NormalMap");
                        if (normalMapTex == -1)
                            normalMapTex = getTexId("_Normalmap");
                occlusionTex = getTexId("_OcclusionMap");
                if (occlusionTex == -1)
                    occlusionTex = getTexId("_AO");
                    if (occlusionTex == -1)
                        occlusionTex = getTexId("Ambient");
