...
#include "Runtime/Engine/Classes/Materials/MaterialExpressionOneMinus.h"
...
public:
	...
	*/
	...
	void DoDecomp(UStaticMesh* StaticMesh, uint32 InHullCount, int32 InMaxHullVerts, uint32 InHullPrecision);
...
//	meshComp->StaticMesh = meshObject;
	...
	if (meshObject){
		...
		for(auto cur: meshObject->StaticMaterials){
			auto mat = cur.MaterialInterface->GetMaterial();
			if (!mat)
				continue;
...
#undef GETPARAM
	...
	*/
		...
		if (subMeshes){
			...
			for(auto cur: *subMeshes){
				...
				auto processIndex = [&](int32 trigVertIdx){
					...
					if (hasNormals)
						newRawMesh.WedgeTangentZ.Add(unityToUe(getIdxVector3(normalFloats, origIndex)*100.0f));
	...
	if (collisionMesh == nullptr)
	{
		...
		mesh->BodySetup->InvalidatePhysicsData();
		mesh->BodySetup->CreatePhysicsMeshes();
		mesh->bCustomizedCollision = true;
	}
	else
	{
		mesh->BodySetup->CopyBodyPropertiesFrom(collisionMesh->BodySetup);
		mesh->BodySetup->CollisionTraceFlag = ECollisionTraceFlag::CTF_UseDefault;
		// Invalidate physics data and create new meshes
		mesh->BodySetup->InvalidatePhysicsData();
		mesh->BodySetup->CreatePhysicsMeshes();
		mesh->bCustomizedCollision = true;
...
*/
	...
	DoDecomp(mesh, 8, 16, 500000);
}

void JsonImporter::DoDecomp(UStaticMesh* StaticMesh, uint32 InHullCount, int32 InMaxHullVerts, uint32 InHullPrecision)
{
...
#undef GETPARAM
	...
	if (useMetallic && !isTransparentQueue){
		UE_LOG(JsonLog, Log, TEXT("Creating metallic value"));
//		createMaterialSingleInput(material, metallic, material->Metallic);
		UMaterialExpressionTextureSample *metallicTexExpression = 0;
		createMaterialInput(material, metallicTex, nullptr, material->Metallic, false, &metallicTexExpression);

		if ((metallicTexExpression != nullptr) && (metallicTexExpression->Outputs.Num() == 5))
		{
			metallicTexExpression->ConnectExpression(&material->Metallic, 1);
			auto oneMinusNode = NewObject<UMaterialExpressionOneMinus>(material);
			material->Expressions.Add(oneMinusNode);
			metallicTexExpression->ConnectExpression(&oneMinusNode->Input, 4);
			material->Roughness.Expression = oneMinusNode;
		}
	}
	...
	if (!useMetallic)
	{
		if ((specTexExpression != nullptr) && (specTexExpression->Outputs.Num() == 5)) {
			...
			specTexExpression->ConnectExpression(&material->Specular, 1);
			auto oneMinusNode = NewObject<UMaterialExpressionOneMinus>(material);
			material->Expressions.Add(oneMinusNode);
			specTexExpression->ConnectExpression(&oneMinusNode->Input, 4);
			material->Roughness.Expression = oneMinusNode;
...
*/
	...
	if (isTransparentQueue)
	{
		...
		material->TranslucencyLightingMode = TLM_VolumetricDirectional;
	}
	if (isAlphaTestQueue)
...
void JsonImporter::importTexture(JsonObjPtr obj, const FString &rootPath){
	...
	bool isNormalMap = name.EndsWith(FString("_n")) || name.EndsWith(FString("Normals")) || name.Contains(FString("_n_"));
	if (isNormalMap){
	...
	if (isNormalMap){
		...
		UE_LOG(JsonLog, Log, TEXT("Normal map green flipped %s"), *name);
	}
